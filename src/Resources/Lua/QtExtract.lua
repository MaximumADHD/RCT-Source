...
local function dumpresourcedata(outputdir, data, names, tree, version)
	...
	local function dump(path, info, c)
		...
		if c ~= 0 then -- collapse root directory
			path = path .. "/" .. info.name
		end
		...
		if info.directory then
			...
			if info.compressed then
				local buffer = content:sub(5)
				local input = BinaryReader.new(buffer)
...
for i, v in next, list do
	...
	dumpresourcedata(outputdir, unpack(v.offsets))
end
